...
import cardano.semifield.{ProbInstance, Semifield}
import cardano.semifield.syntax._
import org.scalatest.{FlatSpec, Matchers}

class ProbSemifieldTest extends FlatSpec with Matchers {

  implicit val semifield: Semifield[Double] = (new ProbInstance {}).semifieldForProb

  "inject/extract" should "work properly" in {
    semifield.inject(math.E) should be (math.E +- 1e-6)
    semifield.extract(0.0) should be (0.0 +- 1e-6)
